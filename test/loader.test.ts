...
import { _internals, load, loadContent, locateConfig } from "../src/loader.ts";
...
`;
      const result = loadContent("my-project", content);
  ...
  describe("locateConfig()", () => {
    ...
    for (
      ...
      it(`read from the config "${config}"`, async () => {
        ...
        const result = await locateConfig("some/path");
