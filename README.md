...
&Then ("and then") is yet another a target runner.  It's major differentiator is awareness of multi-project (e.g. "monorepo") setups; you can have targets in one project that run targets from another.

Key features:
* Specify which target to run using a path-like syntax, with reasonable defaults
* Depend on a target in another project
* Variables with cascading initialization and target updating
...
A project is defined in a `andthen.yaml` (or `.yml`) file, and consists of (project-level) variables and targets.
...
targets:
  ...
  - 
    ...
    deps:
      - ./common-utils:init-caching     # target "init-caching" located in "./common-utils"
      - -init-tag
...
Things &then would likely support in the future (presented in no particular order):

* [] Target options
    * [] Ignore errors
    * [] Always run, even if preceding targets failed
* [] Conditional target execution
* [] Target templates
* [] More variable evaluation
    * [] Name
    * [] Dependencies
    * [] Description (??)
